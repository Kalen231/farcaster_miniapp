...
- **Проблема**: Бесплатный минт птицы в Base App выдавал "Transaction not found" или "Transaction timeout", хотя транзакция успешно выполнялась на блокчейне.
- **Root Cause**: Bundled AA транзакции через EntryPoint требуют времени для индексации на публичном RPC. Наш API вызывал `getTransactionReceipt` слишком рано.
- **Решение**:
  - **Backend** (`verify-transaction/route.ts`): Добавлена retry логика - 5 попыток с интервалом 2 секунды
  - **Frontend** (`ShopModal.tsx`): Увеличен timeout с 15 до 30 секунд
- **Результат**: Транзакции теперь корректно обнаруживаются после bundling

## [2026-01-19] 🚨 CRITICAL: Bundled AA Transaction Verification Fix
...
- **Root Cause**: Base App использует **bundled AA транзакции** через EntryPoint `handleOps`, где несколько UserOps объединены в одну транзакцию. Декодирование `execute()` из calldata невозможно для `handleOps`.
- **Решение в `verify-transaction/route.ts`**:
  - Для EntryPoint транзакций (`0x5ff137d4...` v0.6, `0x00000000717...` v0.7) **доверяем успешной транзакции**
  - Безопасность обеспечивается: 1) проверка уникальности hash, 2) подтвержденный status SUCCESS
  - Для прокси вызовов пытаемся decode `execute()`/`executeBatch()`, иначе доверяем
- **Философия**: Bundled AA транзакции невозможно верифицировать через calldata decode - внутренние переводы видны только через internal transactions на BaseScan
